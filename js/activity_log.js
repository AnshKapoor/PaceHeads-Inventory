$(document).ready(function () {
  const activityLogTable = $("#activityLogTable").DataTable({
    ...
    columns: [
      ...
      },
      {
        ...
        defaultContent: "",
        render: function (data, type, row) {
          return `<textarea class="notes-editor" data-id="${row.id}" 
                  style="width:100%; min-height:40px;" disabled>${
                    data || ""
                  }</textarea>`;
        },
      },
  ...
  // Edit notes button logic
  $("#editNotesBtn").on("click", function () {
    $(".notes-editor").prop("disabled", false);   // Enable all textareas
    $("#saveNotesBtn").prop("disabled", false);   // Enable Save button
    $(this).prop("disabled", true);               // Disable Edit button to prevent re-click
});
  // --- Details Modal Logic ---
...
function formatLogDetails(actionType, detailsJsonString) {
  ...
  switch (actionType) {
    case "LOGIN":
      ...
      html += `<li><strong>Username:</strong> ${
        details.username_attempt || "N/A"
      }</li>`;
      if (details.ip)
    ...
    case "LOGIN_FAILED":
      ...
      html += "<ul>";
      html += `<li><strong>Attempted Username:</strong> ${
        details.username_attempt || "N/A"
      }</li>`;
      if (details.ip)
        html += `<li><strong>IP Address:</strong> ${details.ip}</li>`;
    ...
    case "PRODUCT_UPDATE":
      ...
      html += "<ul>";
      html += `<li><strong>Product Id:</strong> <span class="highlight">${
        details.product_id || "N/A"
      }</span></li>`;
      if (details.product_name) {
        html += `<li><strong>Product Name:</strong> ${details.product_name}</li>`;
      }
      html += `<li><strong>Editor User ID:</strong> ${
        details.editor_user_id || "N/A"
      }</li>`;
    ...
    case "USER_ADD":
      ...
      html += "<ul>";
      html += `<li><strong>Username:</strong> ${
        details.added_username || "N/A"
      }</li>`;
      html += `<li><strong>Email:</strong> ${
        details.added_email || "N/A"
      }</li>`;
      html += `<li><strong>Role:</strong> ${details.added_role || "N/A"}</li>`;
      html += `<li><strong>Added By:</strong> User ID ${
        details.admin_user_id || "N/A"
      }</li>`;
      html += "</ul>";
      break;
    ...
    case "USER_ADD_FAILED":
      ...
      html += "<ul>";
      html += `<li><strong>Attempted Username:</strong> ${
        details.attempted_username || "N/A"
      }</li>`;
      if (details.error)
    ...
    case "CRON_LOG_CLEANUP":
      ...
      html += `<li><strong>Months Retained:</strong> ${
        details.months_retained || "N/A"
      }</li>`;
...
// Saving changes in notes
$("#saveNotesBtn").on("click", function () {
    let notesUpdates = [];

    $(".notes-editor").each(function () {
        const logId = $(this).data("id");
        const newNotes = $(this).val();
        notesUpdates.push({ id: logId, notes: newNotes });
    });

    $.ajax({
        url: BASE_URL_JS + "dashboard/logs/api/update_notes.php",
        type: "POST",
        contentType: "application/json",
        data: JSON.stringify({ updates: notesUpdates }),
        success: function (response) {
            if (response.success) {
                ...
                // Reset UI: Disable notes again
                $(".notes-editor").prop("disabled", true);
                $("#saveNotesBtn").prop("disabled", true);
                $("#editNotesBtn").prop("disabled", false);
            } else {
                alert("Failed to save changes: " + (JSON.stringify(response) || ""));
            }
        },
        error: function () {

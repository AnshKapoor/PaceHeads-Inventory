    ...
    $('#activityLogTable tbody').on('click', '.view-details-button', function() {
        ...
        if (rowData && rowData.details) {
            try {
                ...
                const formattedHtml = formatLogDetails(rowData.action_type, rowData.details);
...
function formatLogDetails(actionType, detailsJsonString) {
    ...
    switch (actionType) {
        case 'LOGIN':
            ...
            html += `<li><strong>Username:</strong> ${details.username_attempt || 'N/A'}</li>`;
            if (details.ip) html += `<li><strong>IP Address:</strong> ${details.ip}</li>`;
            html += '</ul>';
            break;

        case 'LOGIN_FAILED':
            html += '<p class="detail-title failed"><strong>Failed Login Attempt:</strong></p>';
            html += '<ul>';
            html += `<li><strong>Attempted Username:</strong> ${details.username_attempt || 'N/A'}</li>`;
            if (details.ip) html += `<li><strong>IP Address:</strong> ${details.ip}</li>`;
            html += '</ul>';
            break;
        ...
        case 'LOGOUT':
            ...
            html += '<ul>';
            if (details.user_id) html += `<li><strong>User ID:</strong> ${details.user_id}</li>`;
            if (details.username) html += `<li><strong>Username:</strong> ${details.username}</li>`;
            html += '</ul>';
            break;

        case 'PRODUCT_UPDATE':
            html += '<p class="detail-title updated"><strong>Product Update:</strong></p>';
            html += '<ul>';
            html += `<li><strong>Product ID:</strong> <span class="highlight">${details.product_id || 'N/A'}</span></li>`;
            html += `<li><strong>Editor User ID:</strong> ${details.editor_user_id || 'N/A'}</li>`;
            ...
            if (details.changes && Object.keys(details.changes).length > 0) {
                ...
                for (const fieldName in details.changes) {
                    if (details.changes.hasOwnProperty(fieldName)) {
                        ...
                        const oldValue = change.old === null ? 'NULL' : JSON.stringify(change.old); // Use JSON.stringify for clarity if old is empty string/null
                        const newValue = change.new === null ? 'NULL' : JSON.stringify(change.new);
        ...
        case 'PRODUCT_UPDATE_FAILED':
            html += '<p class="detail-title failed"><strong>Product Update Failed:</strong></p>';
            html += '<ul>';
            html += `<li><strong>Product ID:</strong> ${details.product_id || 'N/A'}</li>`;
            if (details.error_message) html += `<li><strong>Error:</strong> <span class="error-text">${details.error_message}</span></li>`;
            if (details.editor_user_id) html += `<li><strong>Editor User ID:</strong> ${details.editor_user_id}</li>`;
            // You might want to display attempted_fields here too if they are useful for debugging
            html += '</ul>';
            break;

        case 'USER_ADD':
            html += '<p class="detail-title added"><strong>New User Added:</strong></p>';
            html += '<ul>';
            html += `<li><strong>Username:</strong> ${details.added_username || 'N/A'}</li>`;
            html += `<li><strong>Email:</strong> ${details.added_email || 'N/A'}</li>`;
            html += `<li><strong>Role:</strong> ${details.added_role || 'N/A'}</li>`;
            html += `<li><strong>Added By:</strong> User ID ${details.admin_user_id || 'N/A'}</li>`;
            html += '</ul>';
            break;

        case 'USER_ADD_FAILED':
            html += '<p class="detail-title failed"><strong>Failed User Addition:</strong></p>';
            html += '<ul>';
            html += `<li><strong>Attempted Username:</strong> ${details.attempted_username || 'N/A'}</li>`;
            if (details.error) html += `<li><strong>Error:</strong> <span class="error-text">${details.error}</span></li>`;
            if (details.admin_user_id) html += `<li><strong>Attempted By:</strong> User ID ${details.admin_user_id}</li>`;
            html += '</ul>';
            break;

        case 'CRON_LOG_CLEANUP':
            html += '<p class="detail-title system-event"><strong>Log Cleanup Event:</strong></p>';
            html += '<ul>';
            html += `<li><strong>Type:</strong> ${details.type || 'N/A'}</li>`;
            html += `<li><strong>Months Retained:</strong> ${details.months_retained || 'N/A'}</li>`;
            html += `<li><strong>Status:</strong> Success</li>`;
            html += '</ul>';
            break;
        
        case 'CRON_LOG_CLEANUP_FAILED':
            html += '<p class="detail-title system-event failed"><strong>Log Cleanup Failed:</strong></p>';
            html += '<ul>';
            html += `<li><strong>Type:</strong> ${details.type || 'N/A'}</li>`;
            if(details.error) html += `<li><strong>Error:</strong> <span class="error-text">${details.error}</span></li>`;
            html += `</ul>`;
        ...
        default:
            html += '<p class="detail-title default"><strong>Raw Details (Type: ' + actionType + '):</strong></p>';
